...
% 1) In a perfect round shape field, noice can create a consistant
%    traveling wave direction. 
% 2) When ictal wavefront is annihilated, traveling wave direction becomes 
%    chaotic
%
...
Ic = 200; % pA
stim_t = [2 5]; % second
stim_x = [0.5 0.5]; % stimulation center, unit: normalized length unit 
stim_r = 0.05; % stimulation radius, unit: normalized length unit
O.Ext = ExternalInput;
O.Ext.Target = O;
...
if exist('noise_sigma','var')
    O.Ext.Random.sigma = noise_sigma;
else
    O.Ext.Random.sigma = 0;
end
