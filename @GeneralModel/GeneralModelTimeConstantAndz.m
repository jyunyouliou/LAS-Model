classdef GeneralModel < RateNetwork
    ...
    methods % Simulation functions        
        function IndividualModelUpdate(O,dt) 
            ...
            V_inf = O.Input.E - O.z.*O.Input.I + I_ext;
            O.V = V_inf + (O.V - V_inf).*exp(-dt./(O.param.tau_V.*O.U));      
            ...
            tau_z_eff = O.param.tau_z ./ (1 + O.param.tau_z.*O.param.Delta_z.*O.Input.I);
            z_inf = 1 ./ (1 + O.param.tau_z.*O.param.Delta_z.*O.Input.I);
            O.z = z_inf + (O.z - z_inf).*exp(-dt./tau_z_eff);     

            % Update equation 4 (resting membrane potential dynamics) in
            % this version, U is used to calculate effective time constant
